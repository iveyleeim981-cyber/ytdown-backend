    ...
    try:
        ...
        try:
            supabase.table('downloads').insert({
                ...
                'format': fmt, 'quality': 'best',
                'ip_address': request.remote_addr
            }).execute()
        except:
            pass
        return jsonify({'title': title, 'duration': str(duration), 'status': 'ready', 'video_id': video_id})
...
def download():
    ...
    try:
        ...
        if not download_url:
            ...
            params2 = {"quality": "137"} if fmt == 'mp4' else {"quality": "140"}
            res2 = requests.get(endpoint, headers=get_headers(), params=params2, timeout=60)
            result2 = res2.json()
            download_url = result2.get('file') or result2.get('reserved_file') or result2.get('url')
        if not download_url:
            return jsonify({'error': 'No download URL found', 'raw': str(result)}), 500
